...
SORTED_UPDATE_LIST = str('sensor.sorted_available_updates_list')

try:
    sorted_update_list = hass.states.get(SORTED_UPDATE_LIST)
    try:
        if sorted_update_list is None:
            logger_text = "Error - entity {} not found"
            logger.error(logger, logger_text.format(sorted_update_list))
    except LookupError:
        logger.error(logger, "Error - a problem occured looking up the update list entity")
    try:
        if sorted_update_list.attributes.update_list
except:
