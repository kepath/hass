...
def new_2fa_authentication_code_requested(PyiCloud, initial_setup=False):
    ...
    try:
        ...
        alert_msg = (f"{EVLOG_ALERT}Alert > Apple ID Verification is needed > "
                        ...
                        f"ID Verification code has been entered. Do the following:"
                        ...
                        f"{CRLF}"
                        f"{CRLF}To request the code again:"
                        f"{CRLF}1. (Recommended) Open iCloud3 Configuration Wizard, select "
                        f"`Enter/Request an Apple ID Verification Code`, or"
                        f"{CRLF}2. Select `Event Log > Actions > Request Apple ID Verification Code`")
